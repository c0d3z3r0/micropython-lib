...
class date:
    ...
    def fromisocalendar(cls, year, week, day):
        ...
        if not 0 < day < 8:
            raise ValueError(
                "Invalid weekday: {} (range is [1, 7])".format(day))
...
class time:
    ...
    def fromisoformat(cls, time_string):
        ...
        try:
            ...
            raise ValueError(
                'Invalid isoformat string: {!r}'.format(time_string))
    ...
    def _name_from_offset(delta):
        ...
        if microseconds:
            return ('UTC{sign}{hours:02d}:{minutes:02d}:{seconds:02d}'
                    '.{microseconds:06d}'.format(
                        sign=sign, hours=hours, minutes=minutes,
                        seconds=seconds, microseconds=microseconds))
        if seconds:
            return 'UTC{sign}{hours:02d}:{minutes:02d}:{seconds:02d}'.format(
                    sign=sign, hours=hours, minutes=minutes, seconds=seconds)
        return 'UTC{sign}{hours:02d}:{minutes:02d}'.format(
                sign=sign, hours=hours, minutes=minutes)
